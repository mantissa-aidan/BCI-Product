...
#    print (yts * 100)
    ...
    left_prob = str(round(yts[0][0] * 100, 2))
    nothing_prob = str(round(yts[0][1] * 100, 2))
    right_prob = str(round(yts[0][2] * 100, 2))

    s1.set('Probability of word: \n' + "left " + left_prob + "% \nnothing " + nothing_prob + "% \nright " + right_prob + "%")
    top.update()
    return
...
def record():
    for i in range(100):
        ...
        while True:    
            ...
            silent = is_silent(input_value, 100)
            if not silent:
                break
...
s1 = Tk.StringVar()
s1.set('You said: ')
L0 = Tk.Label(top, text = 'BCI Call Button (voice prototype)', font = (None, 30))
